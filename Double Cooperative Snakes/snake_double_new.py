...
class SnakeMultiAgentEnv(gym.Env):
    ...
    def __init__(self, width=500, height=500, rows=7, cols=7, headless=True):
        super(SnakeMultiAgentEnv, self).__init__()
...
if __name__ == "__main__":
    for i in range(20):
        # Set headless=True for training on remote server, False for local visualization
        env = SnakeMultiAgentEnv(width=600, height=600, rows=10, cols=10, headless=True)

        observation = env.reset()
        rewards = [0, 0]

        for step in range(100):
            # Skip rendering in headless mode (or it will do nothing anyway)
            # env.render()
            action = env.action_space.sample()  # Take random actions for both snakes
            observation, reward, done, info = env.step(action)
            rewards = [rewards[0] + reward[0], rewards[1] + reward[1]]
        ...
        print(f"Episode {i+1}: Final Rewards - Snake 1: {rewards[0]:.2f}, Snake 2: {rewards[1]:.2f}")
